...
class MDS:
    ...
    def algorithm(self, distances, weights, x0, phi):
        ...
        for i in range(len(p_array)):
            ...
            v_s_p_i = torch.FloatTensor(np.linalg.pinv(v_s_p.data.cpu().numpy(), p)).cuda()
            ...
            z = torch.matmul(v_s_p_i, torch.transpose(phi_s, 0, 1).data)
    ...
    def compute_mat_b(d_mat, dx_mat, w_mat):
        try:
            ...
            b_mat = torch.zeros(d_mat.data.shape)
