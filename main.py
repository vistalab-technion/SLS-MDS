...
from MDS.BothMDS import BothMDS
...
def main(_args, Type):
    ...
    if Type == 'PyTorch':
        ...
        phi = torch.from_numpy(shape.evecs).type(var_type).to(device)
        ...
        mds = BothMDS(mds_params, device=device)

        # torch variables
        phi_t = torch.from_numpy(shape.evecs).type(var_type).to(device)
        d_mat_t = torch.tensor(d_mat, dtype=var_type, device=device)
        x0_t = torch.tensor(x0, dtype=var_type, device=device)
        # numpy variables
        phi = np.real(shape.evecs)

        new_x, new_x_t = mds.algorithm(d_mat, x0, phi, d_mat_t, x0_t, phi_t)
    ...
    if Type == 'PyTorch':
        ...
        # canonical_form_t.mesh.show()  # works on cpu

    else:
        print("Type should be PyTorch, Numpy or Both")
        raise SystemExit()
    shape.plot_embedding(canonical_form.mesh.vertices)
    # canonical_form.mesh.show()  # works on cpu
...
if __name__ == '__main__':
    ...
    parser.add_argument('--display_every', default=50, help='display every n iterations')
